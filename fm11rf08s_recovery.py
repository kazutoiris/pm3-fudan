...
PROXMARK3_CLIENT_PATH = f"{DIR_PATH}\\client"
DICT_DEF_PATH = f"{PROXMARK3_CLIENT_PATH}\\dictionaries\\{DICT_DEF}"
env = os.environ.copy()
env["QT_PLUGIN_PATH"] = f"{PROXMARK3_CLIENT_PATH}\\libs"
env["HOME"] = PROXMARK3_CLIENT_PATH
env["QT_QPA_PLATFORM_PLUGIN_PATH"] = f"{PROXMARK3_CLIENT_PATH}\\libs"
env["PATH"] = f"{PROXMARK3_CLIENT_PATH}\\libs;{PROXMARK3_CLIENT_PATH}\\libs\\shell;"
env["MSYSTEM"] = "MINGW64"
...
tools = {
    ...
    "staticnested_2x1nt1key": os.path.join(
...
def recovery(
    ...
    for line in out.decode("gbk").split("\n"):
        if "UID:" in line:
            print(line)
            uid = int(line.replace("UID:", "").strip().replace(" ", "")[-8:], 16)
    ...
    if init_check:
        ...
        out, _ = pm3.communicate(cmd.encode())
        for line in out.decode("gbk").split("\n"):
            if "[+]  0" in line:
    ...
    try:
        with open(nonces_with_data.replace("`", "").strip(), "r") as file:
    ...
    for sec in range(NUM_SECTORS + NUM_EXTRA_SECTORS):
        ...
        if (
            ...
            cmd = [
                ...
                f"keys_{uid:08x}_{real_sec:02}_{nt[sec][0]}.dic",
                f"keys_{uid:08x}_{real_sec:02}_{nt[sec][1]}.dic",
            ]
            if debug:
                print(" ".join(cmd))
            subprocess.run(cmd, capture_output=True)
    ...
    for sec in range(NUM_SECTORS + NUM_EXTRA_SECTORS):
        ...
        if ((found_keys[sec][0] == "") ^ (found_keys[sec][1] == "")) and nt[sec][
            0
        ] != nt[sec][1]:
            if found_keys[sec][0] == "":
                key_type_source = 1
            ...
            cmd = [
                tools["staticnested_2x1nt1key"],
                nt[sec][key_type_source],
                found_keys[sec][key_type_source],
                dic,
            ]
            if debug:
                print(" ".join(cmd))
    ...
    if final_check:
        ...
        if debug:
            print(cmd)
        pm3.stdin.write(cmd, capture=False, quiet=False)
    else:
        show()
        ...
        show(
            "Found keys have been dumped to `" + color(keyfile, fg="yellow") + "`",
            prompt=plus,
        )
        if unknown:
            show(
        ...
        with open(dumpfile, "wb") as f:
            for sec in range(NUM_SECTORS):
                for b in range(4):
                    ...
                    f.write(bytes.fromhex(d))
        show(
            "Data has been dumped to `" + color(dumpfile, fg="yellow") + "`",
            prompt=plus,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="A script combining staticnested* tools "
    ...
    parser.add_argument(
        ...
        "--init-check",
        action="store_true",
        help="Run an initial fchk for default keys",
    )
    parser.add_argument(
        "-y",
        "--final-check",
        action="store_true",
        help="Run a final fchk with the found keys",
    )
    parser.add_argument(
        "-k",
        "--keep",
        action="store_true",
        help="Keep generated dictionaries after processing",
    )
    ...
    parser.add_argument(
        ...
        "--supply-chain",
        action="store_true",
        help="Enable supply-chain mode. " "Look for hf-mf-XXXXXXXX-default_nonces.json",
    )
    parser.add_argument(
        "-p",
        "--port",
